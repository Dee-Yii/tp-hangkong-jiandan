...
class TradesController extends Controller {
    ...
    */
    ...
    public function getList(){
      ...
      $count = $Trades->join('actuals_goods ON actuals_goods.id = his_trades_record.code_id')->where($map)->count();// 查询满足要求的总记录数
      $list = $Trades->join('actuals_goods ON actuals_goods.id = his_trades_record.code_id')->where($map)->order('open_position_time')->page($page,$pageNum)->select();//获取分页数据
    ...
    public function keep(){
      ...
      $list = $Trades->where($map)->order('depositTime ')->page($page,$pageNum)->select();//获取分页数据
      $Page       = new \Think\Page($count,$pageNum);// 实例化分页类 传入总记录数和每页显示的记录数(25)
      $data['totalPages'] = $count;
      $data['pageNum'] =$pageNum;
      $data['page'] = $page;
